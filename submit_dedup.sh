...
py-spy record -o ${NAME}_${IDX}.svg --rate 1 -- python /hpc/compgen/projects/gw_cfdna/gw_cyclomics/analysis/lchen/FragmentOmics/cyclomicsConsensus/dedup_tidehunter_tagged.py --read_bam /hpc/compgen/projects/gw_cfdna/gw_cyclomics/analysis/lchen/concall_all/concall_github/concall-working/concall/output/$NAME/05_aggregated/${NAME}_tide_rotated.tagged.sorted.bam --SM ${NAME} --out_path /hpc/compgen/projects/gw_cfdna/gw_cyclomics/analysis/lchen/FragmentOmics-output/dedup_by_coordinate/ --prefix ${NAME}_${IDX} --ref /hpc/cog_bioinf/GENOMES.old/Homo_sapiens.GRCh37.GATK.illumina/Homo_sapiens.GRCh37.GATK.illumina.fasta
...
dmesg > ./dmesg_log/dmesg-${NAME}_${IDX}_$!.log
now=$(date +"%T")
echo "Current time : $now"
